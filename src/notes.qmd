---
title: "The Political Landscape of Bremen, A Data Science Approach"
...
To analyze the political landscape of Bremen using dimensionality reduction techniques such as PCA, MDS, and T-SNE, we prepared our datasets with a focus on voter behavior patterns. The raw data from the ![](../img/stmme.png){fig-align="center"}
...
print(f"There are a total of {df_2011.shape[0]} voters in the 2011 Bremen data set of which {Valid_count_2011} successfully cast their votes, there were {df_2011.shape[0] - Valid_count_2011} invalid votes")
...
'''
# Prepare the tables
table_2011_party = tabulate(top_merged_df_2011, headers='keys', tablefmt='fancy_grid')
table_2015_party = tabulate(top_merged_df_2015, headers='keys', tablefmt='fancy_grid')
table_2019_party = tabulate(top_merged_df_2019, headers='keys', tablefmt='fancy_grid')
table_2023_party = tabulate(top_merged_df_2023, headers='keys', tablefmt='fancy_grid')
'''
titles_side_by_side = ["Bremen 2011", "Bremen 2015"]
titles_beneath = ["Bremen 2019", "Bremen 2023"]


# Titles for the tables
titles = ["Bremen 2011", "Bremen 2015", "Bremen 2019", "Bremen 2023"]
...
df_2011_party_percentages.head(10)
```
...
df_2011_party_pairs_ratio.head(10)
...
```{python, echo=FALSE}
matrix_array = matrix_bre_2011.to_numpy()
matrix_array
```
...
'''
matrix_bre_2019_ratio =matrix_bre_2019_ratio.astype(float)
perform_pca_new(matrix_bre_2011_ratio, bre_2011_label_votes, title='Bremen Voter Ratio 2011')
perform_pca_new(matrix_bre_2015_ratio, bre_2015_label_votes, title='Bremen Voter Ratio 2015')
perform_pca_new(matrix_bre_2019_ratio, bre_2019_label_votes, title='Bremen Voter Ratio 2019')
perform_pca_new(matrix_bre_2023_ratio, bre_2023_label_votes, title='Bremen Voter Ratio 2023')
'''
# Define subplots
fig, axs = plt.subplots(2, 2, figsize=(12, 12))
...
In the second Figure, *Bremen Voter Ratio 2015* very much similar to the first one, we can describe the forst principle as the size of the political party with the biggest parties like SPD, CDU, GRÜNE and Die Linke all situated on the far right of the first principal component. the second component again can be used to describe the demographic appeal of the party.
...
# Perform MDS and plot in subplots
plot_mds_with_labels_and_sizes(matrix_bre_2011_ratio, bre_2011_label_votes, title='MDS on Union to Intersection Ration Bremen 2011', ax=axs[0, 0])
plot_mds_with_labels_and_sizes(matrix_bre_2015_ratio, bre_2015_label_votes, title='MDS on Union to Intersection Ration Bremen 2015', ax=axs[0, 1])
plot_mds_with_labels_and_sizes(matrix_bre_2019_ratio, bre_2019_label_votes, title='MDS on Union to Intersection Ration Bremen 2019', ax=axs[1, 0])
plot_mds_with_labels_and_sizes(matrix_bre_2023_ratio, bre_2023_label_votes, title='MDS Union to Intersection Ration Bremen 2023', ax=axs[1, 1])

plt.tight_layout()
plt.savefig('MDS on Union to Intersection Ration.png')
plt.show()
```
...
# Perform t-SNE and plot in subplots
perform_and_plot_tsne(matrix_bre_2011_ratio, bre_2011_label_votes, title='TSNE on Union to Intersection Ration Bremen 2011', ax=axs[0, 0])
perform_and_plot_tsne(matrix_bre_2015_ratio, bre_2015_label_votes, title='TSNE on Union to Intersection Ration Bremen 2015', ax=axs[0, 1])
perform_and_plot_tsne(matrix_bre_2019_ratio, bre_2019_label_votes, title='TSNE on Union to Intersection Ration Bremen 2019', ax=axs[1, 0])
perform_and_plot_tsne(matrix_bre_2023_ratio, bre_2023_label_votes, title='TSNE on Union to Intersection Ration Bremen 2023', ax=axs[1, 1])

plt.tight_layout()
plt.savefig('TSNE on Union to Intersection Ratio.png')
plt.show()
```
